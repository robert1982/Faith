...
function jazzy_forms($, form_id, graph) {
    ...
    function update_output(id) {
        ...
        else if(id == 'sellingcosts'){
            var costs = Number(result.text()) - (Number(element('price').val()) * 0.0034);
            costs = costs + Number(element('deedtax').val());
            element(id).val(costs);
        }
        else if(id == 'proceeds'){
            var proce = Number(element('price').val()) - Number(element('sellingcosts').val());
            element(id).val(proce);
        }
        else {
            element(id).val(result.text());
        }
    }
